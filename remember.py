...
bot = telebot.TeleBot("1795962380:AAELxzh6guN1htz7T6F-nDe59tEEzOauuyI")
...
def answer(message):
    if message.chat.type == 'private':
        ...
        if message.text == "Water":
            ...
            btn2 = types.KeyboardButton(text='60s')
            markup_set_time.add(btn1, btn2)
            photo = open('images/water.jpeg', 'rb')
            bot.send_photo(message.chat.id, photo)
        ...
        if message.text == "Несие":
            ...
            photo = open('images/credit.jpg', 'rb')
            bot.send_photo(message.chat.id, photo)
            bot.send_message(message.chat.id, "Okay you choose Несие!\n"
